...
{   char msg[128];
    ...
    if(image1 !=NULL)
    {   sprintf(msg, "%s: about to update input image1\n", FilterName());
        Logger::LogEvent(msg);
        image1->Update();
        sprintf(msg, "%s: done updating input image1 \n", FilterName());
        Logger::LogEvent(msg);
        
    }
    if(image2 !=NULL)
    {
        sprintf(msg, "%s: about to update input image2\n", FilterName());
        Logger::LogEvent(msg);
        image2->Update();
        sprintf(msg, "%s: done updating input image2\n", FilterName());
        Logger::LogEvent(msg);
...
{   
    ...
    {
        char msg[128];
        sprintf(msg, "%s: no input image1", SinkName());
        DataFlowException e(SinkName(), msg);
        throw e;
    }
...
void TBCombine::Execute()
{
    ...
    if (image2 == NULL) {
        char msg[128];
        sprintf(msg, "%s: no input image2!", SinkName());
        DataFlowException e(SinkName(), msg);
        throw e;
    }
    ...
    for (int i =0; i< image2->GetHeight(); i++)
    {
        for(int j=0; j< image.GetWidth(); j++)
        {
                ...
                int position_lower = 3*(i*image.GetWidth()+j);
    ...
    for(int i=0; i< image1->GetHeight(); i++)
    {
        for(int j=0; j< image1->GetWidth(); j++)
        {
           // int position_rotate = 3*(i*image.GetWidth()+j);
            ...
            int position_rotate = 3*(j*image.GetWidth() + image.GetWidth()-(i+1));
            int position_input = 3*(i*image1->GetWidth()+j);
